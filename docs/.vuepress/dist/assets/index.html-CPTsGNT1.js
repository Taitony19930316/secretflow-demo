import{_ as C,c as w,b as x,a as r,o as M}from"./app-bkoKVQvL.js";typeof window<"u"&&(window.runMPC=async function(){const a=parseFloat(document.getElementById("dataA").value)||100,s=parseFloat(document.getElementById("dataB").value)||200,e=parseFloat(document.getElementById("dataC").value)||300,o=document.getElementById("result");o.innerHTML=`
      <div style="text-align: center; padding: 30px; font-size: 1.2em; color: #666;">
        🔄 正在进行秘密分享和联合计算...
      </div>
    `,await new Promise(y=>setTimeout(y,1e3));const n=Math.floor(Math.random()*a),i=Math.floor(Math.random()*(a-n)),c=a-n-i,l=Math.floor(Math.random()*s),d=Math.floor(Math.random()*(s-l)),h=s-l-d,t=Math.floor(Math.random()*e),p=Math.floor(Math.random()*(e-t)),u=e-t-p,g=n+l+t,v=i+d+p,b=c+h+u,m=g+v+b,f=(m/3).toFixed(2);o.innerHTML=`
        ...
        <div style="background: white; padding: 20px; border-radius: 10px; margin: 15px 0; border-left: 5px solid #764ba2;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; font-size: 1em;">
            ...
            <div style="background: #f1f8e9; padding: 15px; border-radius: 8px;">
              ...
              <div style="color: #555;">a₂(${i}) + b₂(${d}) + c₂(${p})</div>
